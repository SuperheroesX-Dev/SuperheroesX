package mini.sx.entity;

import com.google.common.collect.Iterables;
import io.netty.buffer.ByteBuf;
import mini.sx.SuperheroesX;
import mini.sx.init.EnchantmentInit;
import mini.sx.util.helpers.ItemStackHelper;
import mini.sx.util.helpers.PlayerHelper;
import mini.sx.util.misc.DataWatcherItemStack;
...
public class EntityCaptainAmericasShield extends Entity implements IEntityAdditionalSpawnData, IProjectile {
    ...
    public static WeakHashMap<Object, WeakReference<EntityCaptainAmericasShield>> getBoomerangOwners(World worldIn) {
    ...
    public static void move(EntityCaptainAmericasShield entityBoomerang, double motionX, double motionY, double motionZ) {
    ...
    public void onUpdate() {
        ...
        distanceMooved = getHome().distanceTo(startVec);
    ...
    public Entity getOwner() {
        WeakHashMap<Object, WeakReference<EntityCaptainAmericasShield>> boomerangOwners;
        Entity entity;
        if (world.isRemote) {
            int i = dataManager.get(DATAWATCHER_OWNER_ID);
            entity = world.getEntityByID(i);
            boomerangOwners = EntityCaptainAmericasShield.shieldOwnersClient;
        } else {
            if (owner == null) return null;
            entity = ((WorldServer) world).getEntityFromUuid(owner);
            boomerangOwners = EntityCaptainAmericasShield.shieldOwners;
        }

        if (entity != null) {
            WeakReference<EntityCaptainAmericasShield> reference = boomerangOwners.get(entity);
